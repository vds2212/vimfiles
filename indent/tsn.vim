...
setlocal indentexpr=GetTsnIndent()
" Define keys that trigger indentation:
setlocal indentkeys=0(,0),0[,0],!^F,o,O,e

" Only define the function once.
if exists("*GetTsnIndent")
  finish
endif
...
function GetTsnIndent()
  ...
  if col > 0 && !s:IsInString(v:lnum, col)
    ...
    let pairline = searchpair(pairstart, '', pairend, 'bW', "synIDattr(synID(line('.'), col('.'), 0), 'name') =~? '\\(tsnString\\|tsnComment\\)'")
    " echom "  " . pairline
